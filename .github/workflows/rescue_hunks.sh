...
# ---------------------------------------------------------------------
# 4. 修复 kernel/sys.c (精准处理厂商定制版 newuname 导致的冲突)
# ---------------------------------------------------------------------
SYS_FILE="kernel/sys.c"
if [ -f "$SYS_FILE" ]; then
    echo "[+] Patching $SYS_FILE (Injecting uname spoof hook)..."
    ...
    # 1. 在 SYSCALL_DEFINE1(newuname) 函数入口上方精准注入 extern 声明
    ...
    # 2. 匹配到函数体内部的 memcpy 语句，并在其下方注入劫持逻辑（无视厂商后续的 bpfloader 改动）
    in_func == 1 && /memcpy\(&tmp, utsname\(\), sizeof\(tmp\)\);/ {
        print $0
        print "#ifdef CONFIG_KSU_SUSFS_SPOOF_UNAME"
        print "\tif (static_branch_likely(&susfs_is_uname_spoof_buffer_set))"
        print "\t\tsusfs_spoof_uname(&tmp);"
        print "#endif"
        in_func = 0 # 劫持成功，闭合状态机
        next
    }

    # 兜底防止极端情况下状态机无法闭合，遇到下一个系统调用定义则强行重置
    /^SYSCALL_DEFINE/ {
        in_func = 0
    }
    ...
    echo "[+] $SYS_FILE patched successfully via StateMachine-awk."
fi
